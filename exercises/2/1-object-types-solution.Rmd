...
```{r}
vector1 <- c(1, 3, 5.5, 7.5, 10, 2, 8.5, 9)

vector2 <- c("apple", "banana", "orange", "raspberry", "peach", "cherry", "kiwi", "pear") # this one is slightly harder!
```
...
*Note: any of the following is fine!*
...
*`vector2.num` is now filled with `NA` values*

```{r}

vector2.num <- as.numeric(vector2)
vector2.num
...
*Yes the two vectors are the same length*
...
Q2.4. First, create a vector called `names` containing two elements: the first is the new name for column 1, and the second is the new name for column 2. You can choose to name them whatever you like. What type of vector will you be creating here? Show the relevant code below.

*This should be a character vector with string entries*
...
*There are 8 observations in the df*
